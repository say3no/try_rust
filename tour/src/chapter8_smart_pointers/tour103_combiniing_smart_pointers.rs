 ...
 * 内部データを変更するための不変データ型(複数のスマートポインタが所有している可能性がある)の使用。
 * これはRustでは「内部変異性(omterior mutability」パターンと呼ばれています。
 * これは、Rustのコンパイル時のチェックと同じレベルの安全性で、実行時にメモリ使用量のルールを曲げることができるパターンです
 */
fn main() {
